...
import { GamesGateway } from '../../src/presentation/gateway/games.gateway';
...
class MockGamesGateway {
  ...
  emitRoundStateChange(roundId: string, state: RoundState, crashPoint: number | null) {
    this.events.push({ event: 'round:state-changed', data: { roundId, state, crashPoint } });
  }

  emitMultiplierUpdate(roundId: string, multiplier: number, state: RoundState) {
    this.events.push({ event: 'round:multiplier-updated', data: { roundId, multiplier, state } });
  }

  emitRoundCrashed(roundId: string, multiplier: number, stats: any) {
    this.events.push({ event: 'round:crashed', data: { roundId, multiplier, stats } });
  }

  emitBetPlaced(roundId: string, bet: any) {
    this.events.push({ event: 'round:bet-placed', data: { roundId, bet } });
  }

  emitBetCashedOut(roundId: string, bet: any) {
...
class MockRabbitMQPublisher {
  ...
  async publishBetPlaced(data: any) {
    this.events.push({ type: 'BetPlaced', data });
  }

  async publishBetCashedOut(data: any) {
    this.events.push({ type: 'BetCashedOut', data });
  }

  async publishBetLost(data: any) {
