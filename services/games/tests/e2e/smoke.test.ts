...
async function pollUntil<T>(
	...
	throw new Error(
		`Poll timeout after ${timeoutMs}ms. Last value: ${JSON.stringify(lastValue)}`,
	);
}
...
describe("E2E Smoke Test - Distributed System Integration", () => {
	...
	it(
		"should complete full win flow with balance consistency",
		async () => {
			...
			console.log(
				`[Win] Created wallet: ${userId} with balance ${INITIAL_BALANCE}`,
			);
			...
			console.log(
				`[Win] Created round: ${createRoundRes.data.id} in BETTING state`,
			);
			...
			console.log(
				`[Win] Round is RUNNING (multiplier: ${runningRound.data?.currentMultiplier})`,
			);
			...
			console.log(
				`[Win] Balance consistency: ${INITIAL_BALANCE} - ${BET_AMOUNT} + ${WINNINGS} = ${finalBalance} ✅`,
			);
	...
	it(
		"should complete loss flow with balance consistency",
		async () => {
			...
			expect(createWalletRes.data.balanceInMainUnit).toBe(INITIAL_BALANCE);
			console.log(
				`[Loss] Created wallet: ${userId} with balance ${INITIAL_BALANCE}`,
			);
			...
			expect(crashedRound.data.state).toBe("CRASHED");
			console.log(
				`[Loss] Round crashed: ${crashedRound.data.crashPointMultiplier}x`,
			);

			// Step 5: Poll wallet until debited
			const debitedBalance = INITIAL_BALANCE - BET_AMOUNT;
			console.log(
				`[Loss] Polling for debit: expected balance = ${debitedBalance}`,
			);
			...
			expect(walletAfterDebit.data.balanceInMainUnit).toBe(debitedBalance);
			console.log(
				`[Loss] Wallet after crash: ${walletAfterDebit.data.balanceInMainUnit} (no credit)`,
			);
			...
			console.log(
				`[Loss] Balance consistency: ${INITIAL_BALANCE} - ${BET_AMOUNT} = ${expectedFinal} ✅`,
			);
