...
async function request(url: string, options: any = {}): Promise<{ status: number; data: any }> {
...
describe('Games E2E', () => {
  ...
  describe('GET /games/history', () => {
    it('should return round history', async () => {
      const { status, data } = await request(`${GAMES_URL}/games/history?page=1&limit=10`);
      expect(status).toBe(200);
      expect(Array.isArray(data)).toBe(true);
    });

    it('should validate pagination', async () => {
      const { status } = await request(`${GAMES_URL}/games/history?page=0&limit=10`);
      expect(status).toBeGreaterThanOrEqual(400);
    });
  });
});
