...
export class VerifyRoundUseCase {
  ...
  constructor(@Inject('IRoundRepository') private readonly roundRepository: IRoundRepository) {}
  ...
  async execute(roundId: string): Promise<VerifyRoundResponse> {
    ...
    if (!round.hasCrashed()) {
      throw new Error(`Round ${roundId} is not yet crashed. Cannot verify ongoing rounds.`);
    }
    ...
    const serverSecret = process.env.CRASH_SERVER_SECRET || 'dev-secret-key-change-in-prod';
    ...
    return {
      ...
      formula: 'multiplier = floor((100 * 2^32 - h) / (2^32 - h)) / 100 where h = HMAC-SHA256(seed)[0:8]',
